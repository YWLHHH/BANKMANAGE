...
public class AccountDel extends JFrame implements ActionListener {
  ...
  public AccountDel() {
	...
	background = new ImageIcon("src/del.jpg");
	...
	passwordTextField.setBounds(585, 343, 200, 25);
    ...
    String infob = "";
    try {
    	String strSQL = "select balance from deposit where card='1111111'";
  ...
  public void actionPerformed(ActionEvent e) {
    try {
      if (e.getSource() == cancelBtn) {
          ...
          try {
        	  String strSQL = "select privelege,status from account where card='33333333'";
              rs = db.getResult(strSQL);
              rs.first();
        ...
        if (cardTextField.getText().trim().equals(""))  new Notify(null, "卡号不能为空！");
          ...
          new Notify(null, "密码不能为空！");
        	...
        	strSQL = "update account set adate=(SELECT CONVERT(VARCHAR(30),now(),9)),status='销户' where card='" +
              cardTextField.getText().trim() + "'";
          if (db.updateSql(strSQL)>0) {
            new Notify(null, "销户成功!");
            this.dispose();
            
            strSQL = "insert into deposit values ((SELECT CONVERT(VARCHAR(30),now(),9)),'"+cardTextField.getText().trim()+"','"+balance+"','0','取款')";
            if (db.updateSql(strSQL)>0) {
