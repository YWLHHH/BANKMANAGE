...
public class AccountAdd extends JFrame implements ActionListener {
  ...
  int pot, len;
  String patternID="^(\\d{6})(\\d{4})(\\d{2})(\\d{2})(\\d{3})([0-9]|X)$";
  String patternTel="[0-9]{11}";
  String patternCard="[0-9]{8}";
  String patternPwd="[0-9]{6}";
  ...
  public AccountAdd(String privilege) {
	...
	if (privilege.equals("客户")) background = new ImageIcon("src/custom_create.jpg");
	else background = new ImageIcon("src/clerk_create.jpg");
    ...
    if (privilege.equals("客户")){
        ...
        amountTextField.setBounds(615, 559, 200, 25);
  ...
  public void actionPerformed(ActionEvent e) {
    if (e.getSource() == cancelBtn) {
      ...
      try {
        ...
        if (cardTextField.getText().trim().equals("")) new Notify(null, "卡号不能为空！"); 
        ...
        else if (IDTextField.getText().length()!=18 && Pattern.matches(patternID, IDTextField.getText())) new Notify(null, "身份证只能为18位且必须是数字！");
        else if (telTextField.getText().length()!=11 && Pattern.matches(patternTel, telTextField.getText())) new Notify(null, "电话只能为11位！");
          ...
          if (db.getResult(strSQL).first()) new Notify(null, "卡号已经存在，请使用另一卡号！");
            ...
            strSQL = "insert into account values(NOW(),'" +
            ...
            if (db.updateSql(strSQL)>0) {
              ...
              strSQL = "insert into deposit values(NOW(),'" +
            		  cardTextField.getText().trim() + "','" + 
                      amount + "','" +
                      amount + "','存款')";
              if (db.updateSql(strSQL)>0) {
