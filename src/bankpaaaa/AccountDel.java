  ...
  public void actionPerformed(ActionEvent e) {
    try {
      if (e.getSource() == cancelBtn) {
        ...
        if (cardTextField.getText().trim().equals(""))  new Notify(null, "卡号不能为空！");
        ...
        else if (!privtemp.equals("个人客户") && MainFrame.privelege.equals("银行职员")) new Notify(null, "ְ职员只能由经理销户!");
          ...
          new Notify(null, "密码不能为空！");
        	...
        	strSQL = "update account set status='销户',adate=now()  where card='" +
              cardTextField.getText().trim() + "'";
          ...
          if (db.updateSql(strSQL)>0) {
            ...
            strSQL = "insert into deposit values (NOW(),'"+cardTextField.getText().trim()+"','"+balance+"','0','取款')";
            System.out.println(strSQL);
            if (db.updateSql(strSQL)>0) {
                new Notify(null, "成功取款:"+balance);
